...
wss.on("connection", (ws, req) => {
  ...
  // Parse call_control_id and callback_url from query string
  // e.g. /stream?call_control_id=xxx&callback_url=https://...
  const urlParams = new URLSearchParams(req.url?.split("?")[1] ?? "");
  let callControlId = urlParams.get("call_control_id") ?? null;
  let callbackUrl   = urlParams.get("callback_url")   ?? null;
  if (callControlId) console.log(`📋 Query params: ccId=${callControlId}, callback=${callbackUrl}`);
  let chunks        = [];
  let analyzing     = false;
  ...
  ws.on("message", async (data) => {
    try {
      ...
      if (msg.event === "connected") {
        ...
        // Extract call_control_id and callback_url from Telnyx stream start event
        // Telnyx sends custom_headers as array [{name,value}]
        const customHeaders = msg.start?.custom_headers ?? [];
        const headerMap = {};
        for (const h of customHeaders) {
          if (h.name && h.value) headerMap[h.name.toLowerCase()] = h.value;
        }
        // Only override from start event if not already set from query params
        callControlId = callControlId
          ?? headerMap["call_control_id"]
          ?? msg.start?.call_control_id
          ?? msg.start?.customParameters?.call_control_id
          ?? msg.call_control_id
          ?? null;
        callbackUrl = callbackUrl
          ?? headerMap["callback_url"]
          ?? msg.start?.customParameters?.callback_url
        ...
        analysisTimer = setTimeout(async () => {
          ...
          // Decode each base64 chunk → raw bytes → concatenate → re-encode as single base64
          // This is critical: joining base64 strings directly produces invalid audio
          const rawBuffers = chunks.map(c => Buffer.from(c, "base64"));
          const combined   = Buffer.concat(rawBuffers);
          const combinedAudio = combined.toString("base64");
          chunks = [];
          console.log(`⏱️  [${callControlId}] 2.5s elapsed — ${rawBuffers.length} chunks, ${combined.length} raw bytes → ${combinedAudio.length} b64 chars`);
        ...
        if (!analyzing) {
          ...
          if (chunks.length === 0) {
            console.log(`🎵 First chunk: encoding=${msg.media?.encoding}, sampleRate=${msg.media?.sampleRate}, size=${payload.length}`);
          }
          chunks.push(payload);
        }
        ...
        if (!analyzing && chunks.length > 0) {
          analyzing = true;
          const rawBuffers2 = chunks.map(c => Buffer.from(c, "base64"));
          const combined2   = Buffer.concat(rawBuffers2);
          const combinedAudio = combined2.toString("base64");
          chunks = [];
          console.log(`⏹️  stop-triggered analysis: ${combined2.length} raw bytes`);
          const result = await analyzeAudio(callControlId || "stream", combinedAudio);

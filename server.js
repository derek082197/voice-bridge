...
wss.on("connection", (telnyxWs) => {
  ...
  function connectOpenAI() {
    ...
    openaiWs.on("open", () => {
      ...
      // Send greeting only after both streamSid and OpenAI are ready
      sendGreeting();
      ...
      try {
        ...
        switch (msg.type) {
          case "response.audio.delta":
            console.log("🔊 Audio delta received, streamSid:", streamSid ? "SET" : "NULL");
            if (!streamSid) { console.log("⚠️ audio delta but streamSid null"); break; }
            if (telnyxWs.readyState === WebSocket.OPEN) {
              // Telnyx requires stream_sid (snake_case) in the media event
              ...
              console.log("🔊 Sent audio chunk to Telnyx");
            }
            break;
          ...
          case "response.created":
            console.log("🎙️ OpenAI generating response...");
            break;
    ...
    try {
      ...
      switch (msg.event) {
        ...
        case "start":
          ...
          firstName = msg.start?.customParameters?.firstName
            ?? msg.start?.customParameters?.first_name ?? "";
          state     = msg.start?.customParameters?.state ?? "";
          console.log(`▶️ SID: ${streamSid} | name: "${firstName}" state: "${state}"`);
          sendGreeting();
          break;

        case "media":
          // Only forward audio AFTER greeting has been sent
          // This prevents caller's ambient audio from interrupting the greeting
...
Then stop talking — transfer will happen.
...
- If asked something you cannot answer: "Let me have a specialist help you with that."`;
}
